    ...
    describe("P2P Contract Executor", () => {
        ...
        async function createDeal(
            ...
            try {
                ...
                const fullInfo = await getFullDealInfo(contract, 0);
            console.log("SELLER:", fullInfo.seller.toString());
    ...
    console.log("BUYER Raw :", BUYER.toRawString());
        ...
        test("should create and fund a deal", async () => {
            ...
            // Get commission pool before funding
            const beforeFunding = await testContract.invokeGetMethod("debug_get_contract_data", []);
            const commissionPoolBefore = beforeFunding.type === "success" ? BigInt(beforeFunding.result[1] as any) : 0n;
            console.log(`Commission pool before funding: ${commissionPoolBefore.toString()}`);
            
            // Calculate commission (3% of deal amount)
            const commissionAmount = (dealAmount * 3n) / 100n;
            const totalAmount = dealAmount + commissionAmount + toNano("0.05"); // Adding extra for gas
            console.log(`Deal amount: ${dealAmount.toString()}`);
            console.log(`Commission amount (3%): ${commissionAmount.toString()}`);
            console.log(`Total funding amount: ${totalAmount.toString()}`);
            
            // Fund the deal with sufficient amount
        ...
        test("should resolve deal in favor of seller", async () => {
            ...
            // Get commission pool before funding
            const beforeFunding = await contract.invokeGetMethod("debug_get_contract_data", []);
            const commissionPoolBefore = beforeFunding.type === "success" ? BigInt(beforeFunding.result[1] as any) : 0n;
            console.log(`Commission pool before funding: ${commissionPoolBefore.toString()}`);
            ...
            await fundDeal(contract, memoText, totalAmount);
            
            // Get commission pool after funding
            const afterFunding = await contract.invokeGetMethod("debug_get_contract_data", []);
            const commissionPoolAfter = afterFunding.type === "success" ? BigInt(afterFunding.result[1] as any) : 0n;
            console.log(`Commission pool after funding: ${commissionPoolAfter.toString()}`);
            
            // Verify commission was added to the pool
            const commissionDifference = commissionPoolAfter - commissionPoolBefore;
            console.log(`Commission difference: ${commissionDifference.toString()}`);
            expect(commissionDifference).toBe(commissionAmount);
        ...
        function getReceivedByAddressFromResult(result: any, address: Address): bigint {
            let total = 0n;
        
            if (result.outMessages) {
                for (const msg of result.outMessages) {
                    const dest = msg.info?.dest;
        
                    // 🔍 Логируем оба адреса
                    console.log("📬 Outgoing msg to:", dest?.toString());
                    console.log("🎯 Expected address :", address.toString());
        
                    if (dest && dest.toString() === address.toString()) {
                        total += msg.info.value.coins;
                    }
                }
            } else {
                console.log("⚠️ No outMessages in result");
            }
        ...
        test("should resolve deal and increase seller's balance", async () => {
            const contract = await createContract();
            const dealAmount = toNano("2");
            const memoText = "deal-to-seller";
            ...
            // Step 2: Fund the deal with correct amount (deal amount + commission)
            // Commission is 3% of deal amount
            const commissionAmount = (dealAmount * 3n) / 100n;
            const totalAmount = dealAmount + commissionAmount + toNano("0.05"); // Adding extra for gas
            console.log(`Deal amount: ${dealAmount.toString()}`);
            console.log(`Commission amount (3%): ${commissionAmount.toString()}`);
            ...
            // Step 3: Resolve in favor of seller
            const memoCell = beginCell().storeStringTail(memoText).endCell();
            const externalBody = beginCell()
                .storeUint(2, 32) // op_resolve_deal
                .storeAddress(MODERATOR) // sender
                .storeRef(memoCell)      // memo ref
                .storeUint(1, 1)         // in favor of SELLER
                .endCell();
                async function run(provider: NetworkProvider) {
                    const sender = provider.sender(); // <- WalletContract
                    await sender.send({
                        to: CONTRACT_ADDRESS,
                        value: toNano("1.5"),
                    });
            ...
            // ✅ This is the correct moment to check balance impact
            const received = getReceivedByAddressFromResult(result, SELLER);
            console.log("RECEIVED by SELLER:", received.toString());
        
            expect(received >= dealAmount).toBe(true); // allow for exact or more
        });
